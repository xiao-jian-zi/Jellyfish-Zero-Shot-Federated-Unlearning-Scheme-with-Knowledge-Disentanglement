...
random_classifier4 = torch.nn.DataParallel(resnet.__dict__[args.arch](), device_ids=[5])
random_classifier5 = torch.nn.DataParallel(resnet.__dict__[args.arch](), device_ids=[5])
...
def train(unlearn_epochs , disentangle_epochs):
    ...
    for epoch in tqdm(range(disentangle_epochs),desc='training',unit='epoch'):
        ...
        for i,data in enumerate(Forget_loader, 0):
            ...
            G_real_outputs, lastconv = classifier(inputs)
    ...
    for epoch in tqdm(range(unlearn_epochs),desc='training',unit='epoch'):
        ...
        random_classifier4.eval()
        random_classifier5.eval()
        losses = []
        for i,data in enumerate(Forget_loader, 0):
            inputs, labels = data
            inputs, labels = Variable(inputs), Variable(labels)
            inputs, labels = inputs.to(device), labels.to(device)
            optimizer.zero_grad()
            G_real_outputs, _ = classifier(inputs)
            teacher_outputs,_ = teacher_classifier(inputs)
            ...
            for i in range(probs.size(0)):
                ...
                G_real_labels[i] = sorted_indices[1].item()
            
            with torch.no_grad():
                G_random_outputs1, _ = random_classifier1(inputs)
                G_random_outputs2, _ = random_classifier2(inputs)
                G_random_outputs3, _ = random_classifier3(inputs)
            G_realinputs_loss =  (
                - criterion(G_real_outputs, labels.long()) *2 
                + criterion(G_real_outputs, G_real_labels.long())
